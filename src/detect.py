...
def plot_contour_side_distances(approx_contour):
    ...
    plt.subplot(1, 2, 1)
    bars = plt.bar(side_labels, side_distances, color=['red', 'green', 'blue', 'orange'][:len(side_distances)])
    plt.axhline(y=mean_distance, color='black', linestyle='--', linewidth=2, label=f'Media: {mean_distance:.2f}')
    ...
    for i, (bar, distance) in enumerate(zip(bars, side_distances)):
        plt.text(bar.get_x() + bar.get_width()/2, bar.get_height() + mean_distance*0.02, 
    ...
    for i, point in enumerate(points):
        plt.annotate(f'P{i+1}', (point[0], point[1]), xytext=(5, 5), 
    ...
    for i in range(len(points)):
        ...
        plt.annotate(f'{side_distances[i]:.1f}', mid_point, 
...
def contour_side_intensity(approx_contour, gray_image, show_plot=False):
    ...
    if show_plot:
        ...
        for rank, idx in enumerate(sorted_idx, 1):
            print(f"{rank:>3} | {side_intensities[idx]:9.1f} | {side_angles[idx]:17.1f}° | {side_inclinations[idx]:16.1f}° | {side_assoc[idx]}")
    if show_plot:
        plt.figure(figsize=(12, 6))
        # Subplot 1: Grafico a barre delle intensità ordinate
        plt.subplot(1, 2, 1)
        bars = plt.bar([side_labels[i] for i in sorted_idx], [side_intensities[i] for i in sorted_idx],
                      color=['red' if side_assoc[i]=='background' else 'green' for i in sorted_idx])
        plt.axhline(y=np.mean(side_intensities), color='black', linestyle='--', linewidth=2, label=f'Media: {np.mean(side_intensities):.1f}')
        ...
        for i, (bar, idx) in enumerate(zip(bars, sorted_idx)):
            plt.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 2, 
                    f'{side_intensities[idx]:.1f}', ha='center', va='bottom')
        ...
        for i in range(len(points)):
            next_i = (i + 1) % len(points)
            mid_point = (points[i] + points[next_i]) / 2
            plt.annotate(f'{side_intensities[i]:.1f}\n{side_inclinations[i]:.1f}°\n{side_assoc[i]}', mid_point, 
                        xytext=(0, 10), textcoords='offset points', 
                        ha='center', fontsize=9, color='red', fontweight='bold')

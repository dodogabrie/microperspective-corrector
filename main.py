...
def main(
    ...
    if image_input_format:
        ...
        if image_input_format not in ["tif", "tiff", "jpg", "jpeg"]:
            raise ValueError(
                "Invalid image input format. Use 'tif', 'tiff', 'jpg', or 'jpeg'."
            )
    ...
    if output_path_thumb is None:
        output_path_thumb = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "../../../../output/thumbs")
        )
    ...
    for i, input_path in enumerate(image_files):
        ...
        print(
            f"Processing {i + 1}/{total_files}: {rel_path} (already exists: {already_exists})"
        )
        if not already_exists:
            ...
            if os.path.exists(quality_dir):
                quality_files = [
                    f for f in os.listdir(quality_dir) if f.endswith(".json")
                ]
                if quality_files:
                    ...
                    try:
                        with open(quality_path, "r") as f:
                            ...
                            if "metadata_comparison" in quality_data:
                                if quality_data["metadata_comparison"].get(
    ...
    info_data["metadata_preservation_rate"] = (
        round(
            info_data["successful_metadata_preservation"]
            / max(1, info_data["processed"])
            * 100,
            2,
        )
        if info_data["processed"] > 0
        else 0
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Process images to remove black borders and warp them (recursive, preserves folder structure)."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Number of pixels for the external border.",
    )
    parser.add_argument(
        "-s",
        "--show_step_by_step",
        default=False,
        action="store_true",
        help="Show step-by-step processing (for debugging).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to save reduced thumbnails (optional).",
    )
    parser.add_argument(
        "-f",
        "--image-input-format",
        type=str,
        default="tif",
        help="Input image format (tif/jpg).",
    )
    parser.add_argument(
        "--no-compression",
        action="store_true",
        help="Disable LZW compression for TIFF files (default: compression enabled).",
    )
    ...
    if output_path_thumb is None:
        output_path_thumb = os.path.join(
            output_dir, "thumb"
        )  # Default to output_dir + thumb if not specified
